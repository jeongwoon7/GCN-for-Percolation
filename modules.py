...
"""------------------------------------------------------------------------
4. Implementation of various pooling methods (Statistical row/column-wise global pooling, global pooling)
------------------------------------------------------------------------"""

def x_to_lat(x,num_rows,num_cols):
    lattice = torch.zeros((num_rows, num_cols))
    for i in range(num_rows):
        for j in range(num_cols):
            lattice[i,j]=x[num_rows*i+j]
    return(lattice)


def transform_batch(batch_data,num_rows,num_cols):
    transformed_batch = []
    ...
    for data in batch_data:
        ...
        mat1_2d = x_to_lat(occ_status,num_rows,num_cols)
        mat2_2d = x_to_lat(bd_info,num_rows,num_cols)

        # global pooling
        v1 = mat1_2d.mean()
        v2 = mat2_2d.mean()
        v_concatenated = torch.cat((v1.unsqueeze(0), v2.unsqueeze(0)))
        transformed_batch.append(v_concatenated.clone().detach())

    return torch.stack(transformed_batch)



class CombinedModel_pool(nn.Module):
    def __init__(self, gcn_model):
        super(CombinedModel_pool, self).__init__()
        self.gcn_model = gcn_model
    ...
    def forward(self, x_gcn, adjacency_matrix):
        ...
        # pooling method
        x = transform_batch(gcn_output,num_rows,num_cols)
        x = self.fc1(x)
        x = self.relu(x)
        x = self.fc2(x)
        x = F.softmax(x,dim=1)
...
"""------------------------------------------------------------------------
5. Implementation of Differential pooling method and corresponding training/validating
...
def test_diff(loader,pc,model,option=True):
    ...
    for data in loader:
        ...
        # output = model(data.x,data.adj)[0] : output of the model (bs,2)
        # output.max(dim=1) finds the maximum values in each row
        # and return the values and the corresponding indices. say, if i have [[-2,-1]] it will return (-1,1)
