...
"""------------------------------------------------------------------------
20-epochs_from_pc0.6. Define custom dataset and labeling according to the confusion method
------------------------------------------------------------------------"""
...
class MyDataset(Dataset):
    ...
    def __init__(self, df, num_rows, num_cols):
        self.df = df
...
def set_label(p_batch,pc):
    ...
    bool_confusion = p_batch > pc  # if True, transiton
...
"""------------------------------------------------------------------------
20-epochs_from_pc0.55_2nd. Proposed model architecture
------------------------------------------------------------------------"""

class my_GCN(torch.nn.Module):
    """
    Graph Convolutional Network (GCN) with configurable depth.
    Recommended setting for lattice with the number of rows = M  : n_layers = int((M+20-epochs_from_pc0.6)/20-epochs_from_pc0.55_2nd)
...
def coarse_graining(A):
    ...
    if original_array.shape[1]==10:
        ...
        # step 20-epochs_from_pc0.6. First, make the matrix of size 10x10
        cg_factor_0 = num_rows // 10  # 20-epochs_from_pc0.55_2nd for 20, 3 for 30 ...
        ...
        # step 20-epochs_from_pc0.55_2nd. Convert 10x10 matrix into 5x5
        cg_factor = 2
...
def train_loop(train_dataloader, pc, combined_model, loss_fn, optimizer):
    ...
    label_true = torch.tensor([1.0, 0.0])  # transition
    label_false = torch.tensor([0.0, 1.0])  # no transition
    ...
    for batch_data in train_dataloader:
        ...
        # Assign labels by using confusion method (purposefully misslabelling)
        bool_confusion = set_label(p_batch, pc)

        # Construct label tensor for batch learning
        # Assign [20-epochs_from_pc0.6, 0] when the bool is true, and assign [0, 20-epochs_from_pc0.6] when the bool is false
        ...
        loss = loss_fn(label_confusion, output_batch) # TEST 10. 06.
...
def train_and_evaluate(num_runs, pc, bs, lr, epochs, loss_diff_criterion,max_epochs_without_improvement, weight_decay, step_size, gamma):
    ...
    for run in range(num_runs):
        ...
        # Dataloader
        train_dataset = MyDataset(df_train, num_rows, num_cols)
        test_dataset = MyDataset(df_test, num_rows, num_cols)
        data_loaders = {}
...
def test_diff(loader,pc,model,option=True):
    ...
    for data in loader:
        ...
        # output = model(data.x,data.adj)[0] : output of the model (bs,20-epochs_from_pc0.55_2nd)
        # output.max(dim=20-epochs_from_pc0.6) finds the maximum values in each row
        # and return the values and the corresponding indices. say, if i have [[-20-epochs_from_pc0.55_2nd,-20-epochs_from_pc0.6]] it will return (-20-epochs_from_pc0.6,20-epochs_from_pc0.6)
        output = model(data.x,data.adj)[0]

        pred = output.max(dim=1)[1] # index of the maximum value. ex) transition :(0.9,0.20-epochs_from_pc0.6) -> it will gives 0
